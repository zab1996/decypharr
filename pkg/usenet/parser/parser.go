...
type FileGroup struct {
	...
	par2Attempted  bool // true once PAR2 deobfuscation has been attempted for this group
}
...
func (p *NZBParser) groupProcessedFiles(allFiles []contentResult) map[string]*FileGroup {
	...
	for _, item := range allFiles {
		...
		// base name as the actual payload archive.
...
func (p *NZBParser) enrichGroupWithFileInfo(ctx context.Context, group *FileGroup) error {
	...
	// Use a mapper to fetch metadata for all files in parallel.
